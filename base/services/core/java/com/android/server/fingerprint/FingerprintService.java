...
public class FingerprintService extends SystemService implements IBinder.DeathRecipient {
    ...
    void startAuthentication(IBinder token, long opId, int groupId,
        ...
        if (isSimulated()) {
            ...
            dispatchAuthenticated(547483734688L, 295219575, groupId);
            return;
        }
        /// M: Soter support @}
        IFingerprintDaemon daemon = getFingerprintDaemon();
    ...
    private class ClientMonitor implements IBinder.DeathRecipient {
        ...
        /// M: Soter support @{
        public ClientMonitor(IBinder token, long opId, IFingerprintServiceReceiver receiver,
                int userId, boolean restricted) {
            this(token, receiver, userId, restricted);
            this.opId = opId;
        }
        /// M: Soter support @}
    ...
    public long getAuthenticatorId() {
        /// M: Soter support @{
        if (isSimulated()) return 295219575L;
        /// M: Soter support @}
        IFingerprintDaemon daemon = getFingerprintDaemon();
        if (daemon != null) {
            try {
