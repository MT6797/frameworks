...
final class ProcessList {
    ...
    public static final void setOomAdj(int pid, int uid, int amt) {
        ...
        /// M: ALPS02425865: Check process info of AMS match kernel's process info, @{
        if (Process.getUidForPid(pid) != uid) {
            Slog.w("ActivityManager", "Process " + pid + " get uid is "
                    + Process.getUidForPid(pid)
                    + " does not match uid " + uid + " ignore setting hits oom adj");
            return;
        }
        // @}
