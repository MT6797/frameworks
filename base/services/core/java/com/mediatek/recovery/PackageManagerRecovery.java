...
public class PackageManagerRecovery extends AbstractRecoveryMethod {
    ...
    private final InstallerConnection mInstallerConnection = new InstallerConnection();

    @Override
    public int doRecover(Object param) {
        Slog.d(TAG, "doRecover called, path=" + param);
        mInstallerConnection.waitForConnection();
        ...
        for (int i = 0; i < users.size(); i++) {
            ...
            for (int j = 0; j < files.length; j++) {
                if (mInstaller.changeUidRoot(files[j], user.id) < 0) {
                    Slog.e(TAG,
                            "Change data folder uid to root of package " + files[j] + " failed");
                }
            }
        }
    }
