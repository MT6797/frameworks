...
final class ConnectionServiceAdapterServant {
    ...
    private static final int MSG_NOTIFY_ACCOUNT_CHANGED = MTK_MSG_BASE + 6;
    /* M: CC part end */

    /// M: For 3G VT only @{
    private static final int MSG_NOTIFY_VT_STATUS_INFO = MTK_MSG_BASE + 7;
    /// @}
    ...
    private final Handler mHandler = new Handler() {
        ...
        private void internalHandleMessage(Message msg) throws RemoteException {
            switch (msg.what) {
                ...
                case MSG_NOTIFY_ACCOUNT_CHANGED: {
                    SomeArgs args = (SomeArgs) msg.obj;
                    try {
                        mDelegate.notifyAccountChanged((String) args.arg1,
    ...
    private final IConnectionServiceAdapter mStub = new IConnectionServiceAdapter.Stub() {
        ...
        @Override
        public void notifyAccountChanged(String connectionId, PhoneAccountHandle handle) {
            SomeArgs args = SomeArgs.obtain();
            args.arg1 = connectionId;
            args.arg2 = handle;
            mHandler.obtainMessage(MSG_NOTIFY_ACCOUNT_CHANGED, args).sendToTarget();
        }
        /* M: CC part end */
