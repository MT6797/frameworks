...
final class ConnectionServiceAdapter implements DeathRecipient {
    ...
    void notifyAccountChanged(String callId, PhoneAccountHandle handle) {
        for (IConnectionServiceAdapter adapter : mAdapters) {
            try {
                adapter.notifyAccountChanged(callId, handle);
                Log.d(this, "notifyAccountChanged, callId:%s, handle:%s", callId, handle);
            } catch (RemoteException e) {
                Log.e(this, e, "Exception trying to notify phone account changed!");
            }
        }
    }
    /* M: CC part end */
