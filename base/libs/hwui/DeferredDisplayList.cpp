...
#if DEBUG_DEFER
    #define DEFER_LOGD(...) MLOGD(DEBUG_DEFER, __VA_ARGS__)
#else
    #define DEFER_LOGD(...)
#endif
...
public:
    ...
    virtual void replay(OpenGLRenderer& renderer, Rect& dirty, int index) override {
        ...
        for (unsigned int i = 0; i < mOps.size(); i++) {
            ...
            if (g_HWUI_DEBUG_DISPLAY_LIST_OPS_AS_EVENTS) renderer.eventMark(op->name());
#endif
            DRAW_REPLAY(op->applyDraw(renderer, dirty));
...
public:
    ...
    virtual void replay(OpenGLRenderer& renderer, Rect& dirty, int index) override {
        ...
        if (mOps.size() == 1) {
            DrawBatch::replay(renderer, dirty, -index);
            return;
        }
        ...
        DrawOp* op = mOps[0].op;
#if DEBUG_DISPLAY_LIST_OPS_AS_EVENTS
        if (g_HWUI_DEBUG_DISPLAY_LIST_OPS_AS_EVENTS) {
            renderer.eventMark("multiDraw");
            renderer.eventMark(op->name());
        }
#endif
        MERGEING_DRAW_REPLAY(op->multiDraw(renderer, dirty, mOps, mBounds));
...
public:
    ...
    virtual void replay(OpenGLRenderer& renderer, Rect& dirty, int index) override {
        ...
        int saveCount = -1;
        STATE_OP_REPLAY("StateOpBatch", "S", mOp->applyState(renderer, saveCount));
    }
...
public:
    ...
    virtual void replay(OpenGLRenderer& renderer, Rect& dirty, int index) override {
        ...
        renderer.restoreDisplayState(*mState);
        STATE_OP_REPLAY("RestoreToCountBatch", "R", renderer.restoreToCount(mRestoreCount));
    }
...
void DeferredDisplayList::addDrawOp(OpenGLRenderer& renderer, DrawOp* op) {
    ...
    if (!targetBatch) {
        ...
        DEFER_LOGD("%p creating %sDrawBatch %p, bid %x, at %d",
                this, deferInfo.mergeable ? "Merging" : "",
...
static void replayBatchList(const Vector<Batch*>& batchList,
    ...
    for (unsigned int i = 0; i < batchList.size(); i++) {
        if (batchList[i]) {
            batchList[i]->replay(renderer, dirty, i + 1);
        }
    }
