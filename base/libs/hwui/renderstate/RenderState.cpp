...
void RenderState::render(const Glop& glop) {
    ...
    if (indices.bufferObject == meshState().getQuadListIBO()) {
        ...
        while (elementsCount > 0) {
            ...
            TIME_LOG("glDrawElements", glDrawElements(mesh.primitiveMode, drawCount, GL_UNSIGNED_SHORT, nullptr));
            elementsCount -= drawCount;
            vertexData += (drawCount / 6) * 4 * vertices.stride;
        }
    } else if (indices.bufferObject || indices.indices) {
        TIME_LOG("glDrawElements", glDrawElements(mesh.primitiveMode,
            mesh.elementCount, GL_UNSIGNED_SHORT, indices.indices));
    } else {
