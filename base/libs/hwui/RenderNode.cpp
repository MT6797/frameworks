...
template <class T>
void RenderNode::setViewProperties(OpenGLRenderer& rootRenderer, T& handler) {
    OpenGLRendererWrapper renderer(rootRenderer, true);
#if DEBUG_DISPLAY_LIST
    if (g_HWUI_DEBUG_DISPLAY_LIST) properties().debugOutputProperties(handler.level() + 1);
#endif
...
public:
    ...
    inline void operator()(DisplayListOp* operation, int saveCount, bool clipToBounds) {
        ...
        operation->record(mDeferStruct.mRenderer, saveCount);
    }
...
void RenderNode::defer(DeferStateStruct& deferStruct, const int level) {
    ...
    deferStruct.mRenderer.recorder().addStart(deferStruct.mRenderer, this, mDisplayListData->isEmpty(), mLayer);
    issueOperations<DeferOperationHandler>(deferStruct.mRenderer, handler);
    deferStruct.mRenderer.recorder().addEnd(deferStruct.mRenderer, this);
}
...
public:
    ...
    inline void operator()(DisplayListOp* operation, int saveCount, bool clipToBounds) {
        ...
        if (g_HWUI_DEBUG_DISPLAY_LIST_OPS_AS_EVENTS) mReplayStruct.mRenderer.eventMark(operation->name());
#endif
...
void RenderNode::issueOperationsOf3dChildren(ChildrenSelectMode mode,
        ...
        OpenGLRenderer& rootRenderer, T& handler) {
    OpenGLRendererWrapper renderer(rootRenderer, false);
...
template <class T>
void RenderNode::issueOperationsOfProjectedChildren(OpenGLRenderer& rootRenderer, T& handler) {
    OpenGLRendererWrapper renderer(rootRenderer, false);
...
#if DEBUG_DISPLAY_LIST
                   if (g_HWUI_DEBUG_DISPLAY_LIST) op->output(handler.level() + 1);
#endif
