...
bool Dumper::dumpDisplayList(int width, int height, int frameCount, OpenGLRenderer* renderer) {
    char file[512];
    sprintf(file, "/data/data/%s/dp_%p_%09d.png", mProcessName, renderer, frameCount);

    ALOGD("%s [%s]: %dx%d %s", __FUNCTION__, renderer->recorder().getName(), width, height, file);
    return dumpImage(width, height, file);
}

bool Dumper::dumpDraw(int width, int height, int frameCount, int index,
        OpenGLRenderer* renderer, void* drawOp, int sub) {
    char file[512];
...
bool Dumper::dumpLayer(int width, int height, int fbo, int frameCount,
        OpenGLRenderer* renderer, void* layer) {
    char file[512];
