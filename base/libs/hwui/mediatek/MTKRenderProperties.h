...
class RenderProperties;
    ...
    #define RENDER_PROPERTIES_LOGD(...) MLOGD(debug_render_properties, __VA_ARGS__)
#else
...
#define AP_STATIC_MATRIX        nullptr
#define AP_ANIMATION_MATRIX     nullptr
#define AP_TRANSFORM_MATRIX     nullptr
...
#define AP_SET(GETTER, VALUE, PROPERTY, TYPE, ...) \
        (GETTER() != VALUE ? (VALUE == AP_##PROPERTY ? remove(AP_ID_##PROPERTY) : \
            add(AP_ID_##PROPERTY, sizeof(TYPE), &VALUE) != nullptr, ##__VA_ARGS__, true) : false)
...
#define AP_MUTATE(GETTER, PROPERTY, TYPE) { \
            if (!getAttachProperty(AP_ID_##PROPERTY)) add(AP_ID_##PROPERTY, sizeof(TYPE), nullptr); \
...
public:
    ...
    bool promotedToLayer() const {
        ...
        return layerProperties().mType == LayerType::None
                && getWidth() <= maxTextureSize
                && getHeight() <= maxTextureSize
                && (mComputedFields.mNeedLayerForFunctors
                        || (!MathUtils::isZero(getAlpha())
                                && getAlpha() < 1
                                && getHasOverlappingRendering()));
    }

    LayerType effectiveLayerType() const {
        return CC_UNLIKELY(promotedToLayer()) ? LayerType::RenderLayer : layerProperties().mType;
    }
