...
void CacheTexture::releasePixelBuffer() {
    ...
    if (mTexture.id) {
        ...
        TT_REMOVE(mTexture.id, "[CacheTexture.h] releaseTexture -");
        mTexture.id = 0;
    }
    mDirty = false;
    mCurrentQuad = 0;
...
void CacheTexture::allocatePixelBuffer() {
    ...
    if (!mTexture.id) {
        ...
        TT_ADD(String8("CacheTexture"), mTexture.id, getWidth(), getHeight(),
                mFormat, GL_UNSIGNED_BYTE, String8("font"),
                "[CacheTexture.h] allocateTexture +");
