...
void GradientCache::generateTexture(uint32_t* colors, float* positions, Texture* texture) {
    ...
    if (mUseFloatTexture) {
        ...
        TT_ADD(texture->id, width, texture->height, GL_RGBA, GL_FLOAT,
                "gradient", "[GradientCache.cpp] generateTexture +");
    } else {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, texture->height, 0,
                GL_RGBA, GL_UNSIGNED_BYTE, pixels);
        TT_ADD(texture->id, width, texture->height, GL_RGBA, GL_UNSIGNED_BYTE,
                "gradient", "[GradientCache.cpp] generateTexture +");
    }
