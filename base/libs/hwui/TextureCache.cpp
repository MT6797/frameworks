...
TextureCache::TextureCache()
    ...
    int screenBasedSize = ceil(((lcm_width * lcm_height * 4.0) / (1024 * 1024)) * DEFAULT_TEXTURE_CACHE_SCREEN_COUNT);
    if (!setSizeByProperty) {
        if (screenBasedSize > DEFAULT_TEXTURE_CACHE_SIZE) {
            INIT_LOGD("  Setting texture cache size to %.2fMB by screen based size", screenBasedSize);
            setMaxSize(MB(screenBasedSize));
        }
    }
#endif /* defined(LCM_WIDTH) && defined(LCM_HEIGHT) */
...
void TextureCache::uploadToTexture(bool resize, GLenum format, GLsizei stride, GLsizei bpp,
    ...
    if ((stride == width) || useStride) {
        ...
        if (resize) {
            ...
            TT_ADD(-1, width, height, format, type, "texture", "[TextureCache.cpp] uploadToTexture +");
        } else {
