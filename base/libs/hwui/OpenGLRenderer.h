...
public:
    ...
    void drawScreenSpaceColorRect(float left, float top, float right, float bottom, int color) {
        ...
        bool stencilWasEnabled = mRenderState.stencil().isTestEnabled();
        ...
        if (stencilWasEnabled) mRenderState.stencil().enableTest(0);
        mDirty = true;
    }
#endif
    ...
    DisplayListRecorder& recorder() { return mRecorder; }
...
private:
   ...
   /**
    ...
    /// M: friend clip ops to get current clip region for debug purposes
    friend class ClipRectOp;
    friend class ClipRegionOp;
    friend class ClipPathOp;
