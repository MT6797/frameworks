...
public class NetworkControllerBaseTest extends SysuiTestCase {
    ...
    /// M: for volte case.@{
    protected int mSubIdEx;
    protected MobileSignalController mMobileSignalControllerEx;
    protected PhoneStateListener mPhoneStateListenerEx;
    /// @}
    ...
    protected void setupNetworkController() {
        ...
        setSubscriptions(mSubId, mSubIdEx);
        mMobileSignalController = mNetworkController.mMobileSignalControllers.get(mSubId);
        mPhoneStateListener = mMobileSignalController.mPhoneStateListener;

        /// M: for volte test @{
        mMobileSignalControllerEx = mNetworkController.mMobileSignalControllers.get(mSubIdEx);
        mPhoneStateListenerEx = mMobileSignalControllerEx.mPhoneStateListener;
        Log.d(TAG, "mMobileSignalControllers size:"
                + mNetworkController.mMobileSignalControllers.size());
        /// @}
    }
    ...
    protected void setSubscriptions(int... subIds) {
        ...
        for (int subId : subIds) {
            ...
            when(subscription.getSimSlotIndex()).thenReturn(subs.indexOf(subscription));
        }
    ...
    public void updateDataConnectionStateEx(int dataState, int dataNetType) {
        mPhoneStateListenerEx.onDataConnectionStateChanged(dataState, dataNetType);
    }
    ...
    protected void volteIconTest(int phoneId, int netState, int netType,
            int serviceState, int iconId) {
        // Stubbing collaborators.
        updateDataConnectionState(phoneId, netState, netType);

        // Invocation Ims action.
        mNetworkController.setTest(true);
        mNetworkController.handleIMSAction(createImsStateIntent(serviceState, phoneId));

        // Verify volte icon.
        verifyLastVolteIcon(iconId);
    }

    protected void updateDataConnectionState(int phoneId, int dataState, int dataNetType){
        if ( phoneId == 0){
            updateDataConnectionState(dataState,dataNetType);
        } else {
            updateDataConnectionStateEx(dataState,dataNetType);
        }
    }

    protected void verifyLastVolteIcon(int icon){
        ArgumentCaptor<Integer> iconArg = ArgumentCaptor.forClass(Integer.class);
        Mockito.verify(mCallbackHandler, Mockito.atLeastOnce()).setVolteStatusIcon(
                iconArg.capture());

        assertEquals("volte icon in status bar", icon, (int) iconArg.getValue());
    }
    /// @}
