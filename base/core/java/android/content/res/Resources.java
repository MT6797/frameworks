...
public class Resources {
    ...
    private static final ArrayMap<String, LongSparseArray<ConstantState>> sBoostDrawableCache =
        new ArrayMap<String, LongSparseArray<ConstantState>>();
    /// @}
...
//add by liliang.bao begin
	  ...
	  if(mCustomSystemRes!=null)
	   	mCustomSystemRes.updateCustomResConfiguration(config,  metrics,  null);
            ...
            if (isBoostApp(boostKey)) {
                LongSparseArray<ConstantState> boostCache = sBoostDrawableCache.get(boostKey);
                if (boostCache != null) {
    ...
    private void clearBoostDrawableCacheLocked(LongSparseArray<ConstantState> cache,
            int configChanges) {
        if (DEBUG_CONFIG) {
        ...
        for (int i = 0; i < N; i++) {
            final ConstantState cs = cache.valueAt(i);
            if (cs != null) {
    ...
    Drawable loadDrawable(TypedValue value, int id, Theme theme) throws NotFoundException {
        ...
        if (!mPreloading) {
            ...
            /// M: Boost cache on system @{
            final Drawable boostDrawable = getBoostCachedDrawable(sBoostDrawableCache, key);
        ...
        if (mPreloading) {
            ...
            synchronized (mAccessLock) {
                ...
                if (!isColorDrawable) {
                    final String boostKey = this.toString().split("@")[0];
                    if (isBoostApp(boostKey)) {
                        LongSparseArray<ConstantState> boostCache =
                            sBoostDrawableCache.get(boostKey);
                        if (boostCache == null) {
                            boostCache = new LongSparseArray<ConstantState>(1);
    ...
    /// M: Boost cache on system @{
    private Drawable getBoostCachedDrawable(ArrayMap<String, LongSparseArray<ConstantState>> caches,
            long key) {
        synchronized (mAccessLock) {
            final String boostKey = this.toString().split("@")[0];
            if (isBoostApp(boostKey)) {
                LongSparseArray<ConstantState> boostCache = sBoostDrawableCache.get(boostKey);
                if (boostCache != null) {
                    final Drawable boostDrawable = getBoostCachedDrawableLocked(boostCache, key);
                    if (boostDrawable != null) {
                        return boostDrawable;
                    }
                }
    ...
    private Drawable getBoostCachedDrawableLocked(LongSparseArray<ConstantState> drawableCache,
            long key) {
        final ConstantState entry = drawableCache.get(key);
        if (entry != null) {
