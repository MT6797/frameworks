...
public class DreamService extends Service implements Window.Callback {
   ...
   /**
    ...
    public final IBinder onBind(Intent intent) {
        ...
        Slog.v(TAG, "Class name is: "+ className);
        if((className.contains("PhotoTableDream"))||(className.contains("FlipperDream")))
        {
          if (getApplicationContext().checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE)
          != PackageManager.PERMISSION_GRANTED)
          {
            Slog.v(TAG, "Permission Disabled");
            return null;
          }
          else{
            Slog.v(TAG, "Permission Enabled");
