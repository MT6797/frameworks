...
public final class BluetoothAdapter {
    ...
    /**
     * enable or disable Bluetooth Ssp Debug Mode
     *
     * <p>Requires the {@link android.Manifest.permission#BLUETOOTH_ADMIN}
     * permission
     *
     * @return true to indicate configure Ssp Debug Mode successfully, or false on
     *         immediate error
     * @hide
     */
    public boolean configSspDebugMode(boolean enable){
        try {
            synchronized(mManagerCallback) {
                if (DBG) Log.d(TAG, "mService = " + mService);
                if (mService != null) return mService.configSspDebugMode(enable);
            }
        } catch (RemoteException e) {Log.e(TAG, "", e);}
        return false;
    }
