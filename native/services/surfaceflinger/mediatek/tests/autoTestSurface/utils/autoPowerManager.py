...
class autoPowerManager(object):
    ...
    def _getInfo(self) :
        rst = os.popen('adb -s %s shell wm size' %self._serial).read()
    ...
    def _getState(self) :
        rst = os.popen('adb -s %s shell dumpsys SurfaceFlinger' %self._serial).read()
    ...
    def _power(self) :
        #print 'press power...'
        os.popen('adb -s %s shell input keyevent KEYCODE_POWER' %self._serial)
    ...
    def _unlockScreen(self) :
        ...
        swipe_cmd = 'adb -s %s shell input swipe %d %d %d %d' % (self._serial, start_x, start_y , start_x , start_y-500)
