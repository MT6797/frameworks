...
def run_test_folder(serialNum, rootpath, folder, isDry , verbose, prepare = True) :
    ...
    test_cmd = test_path + '\\run.py %s' %serialNum

    ## run it ##
    if isDry == False :
        if prepare == True :
            externel_cmd('push_data.bat %s' %serialNum, verbose)
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],"f:vhdtnems")
    ...
    if len(autoFolderList) == 0 :
        autoFolderList = ['autoVideoTelephony','autoVDSaddVideo', 'autoVDSvlw', 'autoIonFdLifeCycle']


    root_path = os.path.abspath('.')

    rstFolder = root_path + time.strftime("\\SF_UT_LOG_" + serialNum +"_%y%m%d_%H%M%S")


    os.makedirs(rstFolder)

    logDB = log_db(serialNum)
    #logDB.clean_db()

    logFD = log_fd(serialNum, outfile = rstFolder+'\\logfd.csv')
    logFD.log_level = 0
    logFD.start(async = True)

    if islogIon == True :
        log_ion = log_ion_heap(serialNum, rstFolder+'\\log_ion_heap.csv')
    ...
    for testFolder in autoFolderList :
        ...
        run_test_folder(serialNum, root_path, testFolder, isDryRun, isVerbose, True)
    ...
    if isRunExtEvent == True :
        ...
        apm = autoPowerManager(serialNum)
        ...
        for testFolder in autoFolderList :
            ...
            run_test_folder(serialNum, root_path, testFolder, isDryRun, isVerbose, False)
