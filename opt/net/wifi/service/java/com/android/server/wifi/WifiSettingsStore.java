...
final class WifiSettingsStore {
    ...
    synchronized boolean isWifiToggleEnabled() {
        Slog.d(TAG, "isWifiToggleEnabled in");
        if (!mCheckSavedStateAtBoot) {
                    Slog.d(TAG, "mCheckSavedStateAtBoot");
            mCheckSavedStateAtBoot = true;
            if (testAndClearWifiSavedState()) {
                Slog.d(TAG, "testAndClearWifiSavedState true");
                return true;
            }
        }
...
//        mAirplaneModeOn = getPersistedAirplaneModeOn();
        ...
        Slog.d(TAG, "Received ACTION_AIRPLANE_MODE_CHANGED, mAirplaneModeOn:" + mAirplaneModeOn);
        if (mAirplaneModeOn) {
