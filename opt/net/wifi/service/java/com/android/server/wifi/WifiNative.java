...
//            auth = 0;
    ...
    public boolean p2pRemoveClient(String iface, String mac) {
        return doBooleanCommand("IFNAME=" + iface + " P2P_REMOVE_CLIENT " + mac );
    }
    ...
    public boolean p2pSetBssid(int id, String bssid) {
        if (mInterfaceName.equals("p2p0")) {
            synchronized (mLock) {
                return doBooleanCommandNative("IFNAME=" + mInterfaceName + " SET_NETWORK " + id +
                    " bssid " + bssid);
            }
        }
    ...
    public String wifiLinkStatics() {
        return doStringCommand("DRIVER GET_STA_LINK_DETECT");
    }
     ...
     */
    //set FrameID with IE
    public boolean vendorIEAdd(int frameId, String hex) {
        return doBooleanCommand("IFNAME=" + mInterfaceName + " VENDOR_ELEM_ADD " +
            frameId + " " + hex);
    }

    //get vendor IE with FrameID
    public String vendorIEGet(int frameId) {
        return doStringCommand("IFNAME=" + mInterfaceName + " VENDOR_ELEM_GET " + frameId);
    }

    //remove all/specific vendor IE with FrameID
    public boolean vendorIERemove(int frameId, String hex) {
        if (hex == null) {
            return doBooleanCommand("IFNAME=" + mInterfaceName + " VENDOR_ELEM_REMOVE " +
                frameId + " *");
        } else {
            return doBooleanCommand("IFNAME=" + mInterfaceName + " VENDOR_ELEM_REMOVE " +
                frameId + " " + hex);
    ...
    public boolean doCtiaTestOn(String cmd) {
        if (cmd.equals("scan")
                | cmd.equals("roam")
                | cmd.equals("cam")
                | cmd.equals("bcn")
                | cmd.equals("power")
                | cmd.equals("fifo")) {
            return doBooleanCommand("DRIVER smt-test-" + cmd + "-on");
        } else if (cmd.equals("default")) {
            return doBooleanCommand("DRIVER smt-test-on");
        } else {
            Log.e(mTAG, "doCtiaTestOn wrong cmd: " + cmd);
            return false;
        }
    }

    public boolean doCtiaTestOff(String cmd) {
        if (cmd.equals("scan")
                | cmd.equals("roam")
                | cmd.equals("cam")
                | cmd.equals("bcn")
                | cmd.equals("power")
                | cmd.equals("fifo")) {
            return doBooleanCommand("DRIVER smt-test-" + cmd + "-off");
        } else if (cmd.equals("default")) {
            return doBooleanCommand("DRIVER smt-test-off");
        } else {
            Log.e(mTAG, "doCtiaTestOff wrong cmd: " + cmd);
            return false;
        }
    }
