...
public abstract class IccRecords extends Handler implements IccConstants {
    ...
    */
    ...
    abstract int getChildPhoneId();
    abstract void updatePHBStatus(int status, boolean isSimLocked);
    // PHB Refactoring -----
    // FDN support start
   ...
   // FDN support end
    /**
     * Check the active phone type by phoneId.
     */
    boolean isRightPhone(int phoneId, String type) {
        boolean result = false;
        PhoneBase csPhone = null;
        Phone phone = PhoneFactory.getPhone(phoneId);
        csPhone = (PhoneBase) ((PhoneProxy) phone).getActivePhone();
        String phoneName = csPhone.getPhoneName();
        log("isRightPhone phoneId + " + phoneId + " | type : " + type + " | phoneName : "
                + phoneName);
        result = (type.equals(phoneName));
        return result;
    }
}
