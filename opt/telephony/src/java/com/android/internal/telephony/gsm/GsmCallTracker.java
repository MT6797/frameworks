...
public final class GsmCallTracker extends CallTracker {
    ...
    public void dispose() {
        ...
        // hangup and disconnect handover connections
        for (Iterator<Connection> it = mHandoverConnections.iterator(); it.hasNext();) {
            Connection hoConnection = it.next();
            try {
                hoConnection.hangup();
            } catch (CallStateException ex) {
                Rlog.e(LOG_TAG, "dispose: unexpected error on hangup hoConnection", ex);
            }
            Rlog.d(LOG_TAG, "dispose - disconnect hoConn= " + hoConnection);
            ((ImsPhoneConnection) hoConnection).onDisconnect(DisconnectCause.LOST_SIGNAL);
            it.remove();
        }
        /// @}
        clearDisconnected();
    ...
    1. current phone serviceState is STATE_POWER_OFF
    2, pendingMO is not NULL
    3. both foreground and background call exist (1A1H within current phone)
    */
    boolean
    canDial() {
        boolean ret;
