...
public class DcSwitchStateMachine extends StateMachine {
    ...
    //MTK START
    private static final int CMD_RETRY_DETACH      = BASE + 100;
    //MTK END
                    ...
                    if (mCurrentAllowedSequence != msg.arg1) {
                        ...
                        if (ar.exception instanceof CommandException) {
                            ...
                            if (e.getCommandError() ==
                                    ...
                                    || (isRadioUnavailableException(ar)
                                ...
                                // M: exception when isCapabilitySwitching & isWorldPhoneSwitching
                                // should back to idle than retry attach.
                                retVal = HANDLED;
                                break;
                            }
                        }
    ...
    private class DetachingState extends State {
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case EVENT_DATA_DISALLOWED: {
                    ...
                    if (mCurrentDisallowedSequence != msg.arg1) {
                        ...
                        if (isRadioUnavailableException(ar) &&
                                WorldPhoneUtil.isWorldPhoneSwitching()) {
                            // M: for sim switch case, retry detach
                            sendMessageDelayed(obtainMessage(CMD_RETRY_DETACH), 500);
                        } else {
                            transitionTo(mIdleState);
                        }

                        // M: We assume detach shouldn't fail, only "sim switch case" we need retry.
                        //transitionTo(mAttachedState);
                    } else {
                ...
                case CMD_RETRY_DETACH:
                    if (DBG) {
                        log("DetachingState: CMD_RETRY_DETACH" );
                    }
                    PhoneBase pb = (PhoneBase)((PhoneProxy)mPhone).getActivePhone();
                    pb.mCi.setDataAllowed(false, obtainMessage(EVENT_DATA_DISALLOWED,
                            ++mCurrentDisallowedSequence, 0));
    ...
    private boolean isRadioUnavailableException(AsyncResult ar) {
        ...
        if (ar.exception instanceof CommandException) {
            ...
            if (e.getCommandError() ==
                ...
                log("isRadioUnavailableException: exception is RADIO_NOT_AVAILABLE");
