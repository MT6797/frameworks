...
//!++
    ...
    virtual status_t getExternalDeviceListener(/*out*/sp<IBinder>& listener)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ICameraService::getInterfaceDescriptor());

        remote()->transact(BnCameraService::GET_EXTERNAL_DEVICE_LISTENER, data, &reply);
        status_t err = reply.readInt32();
        listener = (reply.readStrongBinder());
        return err;
    }
#endif
//!--
...
//!++
        ...
        case GET_EXTERNAL_DEVICE_LISTENER: {
            ALOGD("GET_EXTERNAL_DEVICE_LISTENER");
            CHECK_INTERFACE(ICameraService, data, reply);
            //
            sp<IBinder> listener;
            reply->writeInt32(getExternalDeviceListener(listener));
            reply->writeStrongBinder(listener);
            return OK;
        } break;
#endif
//!--
