...
private:
    ...
    bool hit(off64_t offset) {return (mCacheOffset <= offset && offset < mCacheOffset + (int64_t)mCacheTotalLen);}
...
ssize_t FileCache::CacheNode::fill(int fd) {
    ...
    if (result == -1) {
        SLOGE("seek to %lld failed", mOffset);
...
ssize_t FileCache::CacheNode::read(off64_t offset, void *data, size_t size) {
    ...
    if (mData && offset >= mOffset && offset < (mOffset + mDataLen)) {
        ...
        if (pos + sz > mDataLen) {
            SLOGE("pos(%lld)+sz(%d) > mDataLen(%d)",pos, sz, mDataLen);
            return 0;
        }
...
FileCache::FileCache(int fd,
    ...
    if (len) {
        ...
        // DupFd maybe fail for special path, e.g. /data
        if (mDupFd == -1) SLOGE("Open dupFd fail for file %s", filename);
    }
#endif
...
void FileCache::updateCacheNodeList(CACHE_TYPE ct, off64_t offset) {
    ...
    Mutex::Autolock autoLock(mLock);
    SLOGV("updateCacheNodeList, ct = %d, offset = %lld", ct, offset);
    //dumpFileCacheInfo();
...
ssize_t FileCache::read(off64_t offset, void *data, size_t size) {
    ...
    SLOGV("FileCache::read, offest %lld, size %ld, file length %lld", offset, size, mLength);
        ...
        while (it != cn->end()) {
            if ((*it)->hit(readOffset)){
                ...
                if ( readCacheLength < 0) {
                    SLOGD("%s, readCacheLength = %d", __FUNCTION__, readCacheLength);
                    readCacheLength = 0;
                }
    ...
    if (readSize) {
        SLOGV("read from file system, size %d, offset %lld", readSize, readOffset);
// #ifdef USE_CACHE_MMAP
        ...
        ssize_t sz;
        if (result == -1) {
            SLOGV("FileCache, seek to %lld failed", readOffset);
            return UNKNOWN_ERROR;
        }
    ...
    SLOGV("------ FileCache read fd:%ld,offset:%16lld,size:%8ld,time:%16lld",mFd,offset,size,readTime);
...
void FileCache::dumpFileCacheInfo() {
    if (mLittleCacheNodes.size()) {
        ...
        SLOGD("Dump FileCache Information: little cache number: %d, offset: %lld, size: %d", mLittleCacheNodes.size(), offset, size);
    }
    else {
    ...
    if (mBigCacheNodes.size()) {
        ...
        SLOGD("Dump FileCache Information: big cache number: %d, offset: %lld, size: %d", mBigCacheNodes.size(), offset, size);
    }
    else {
