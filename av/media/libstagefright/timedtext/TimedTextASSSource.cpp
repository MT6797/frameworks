...
status_t TimedTextASSSource::scanFile(){
    ...
    //ALOGE("[ASS Parser] Ass File Scan Over! vector size =%x" , mTextVector.size());
...
//  ALOGE("[TXT Parser] getText vector size=%d, sLangIndex=%d", mTextVector.size(), selectedLangIndex);
    ...
    if ((size_t)mCurIndex >= mTextVector.size()) {
        return ERROR_END_OF_STREAM;
    }
    ...
    *endTimeUs = info.endTimeUs;
    ...
    if (FileCacheManager::getInstance().readFromCache(mExSource, info.offset, str, info.textLen) < (uint64_t)info.textLen) {
        delete[] str;
        return ERROR_IO;
    }
