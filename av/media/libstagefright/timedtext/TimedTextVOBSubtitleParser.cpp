...
{
    UNUSED(dataPacketBuffer);
    UNUSED(dataPackageSize);
...
stGenerateBitmapLine(int & offset,char * bitmapBuffer)
{
    ...
    do
    {
        ...
        if (OK == status)
        {
            ...
            {
                ALOGE("[Ruiiiiii]remain Pixel Count less than draw count %d > %d \n", remainPixelCount, count);
            }
...
stPrepareBitmapBuffer()
{
    ...
    for (int i=0;i < VOB_TMP_FILE_COUNT; i++)
    {
        ...
        m_iFd[i] = open(filepath, O_CREAT | O_RDWR | O_TRUNC, 0777);
