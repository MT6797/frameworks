...
#define AAC_EXTR_VEB(fmt, arg...) SLOGV(fmt, ##arg)
#define AAC_EXTR_DBG(fmt, arg...) SLOGD(fmt, ##arg)
#define AAC_EXTR_INFO(fmt, arg...) SLOGI(fmt, ##arg)
#define AAC_EXTR_WARN(fmt, arg...) SLOGW(fmt, ##arg)
#define AAC_EXTR_ERR(fmt, arg...) SLOGE("Err: %5d:, " fmt, __LINE__, ##arg)
#else
...
MtkAACExtractor::MtkAACExtractor(
    ...
    if (mDataSource->getSize(&fileSize) == OK) {
        if (bIsADIF) {
            ...
            ALOGI("ADIF: fileSize = %d, nDurationUs = %llu", fileSize, (unsigned long long)nDurationUs);
        } else {
            off64_t numFrames = 0;
            off_t nextPos = pos;
            while (pos < fileSize) {
