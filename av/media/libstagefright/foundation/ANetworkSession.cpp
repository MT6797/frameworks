...
#ifdef MTK_CROSSMOUNT_SUPPORT
            if (localAddr != NULL) {
                notify->setInt64("RtspClientIp", (int64_t)ntohl(localAddr->s_addr));
                ALOGI("Save client IP %ld", (long)ntohl(localAddr->s_addr));
            }
#endif
            break;
...
bool ANetworkSession::isClientAllowToConnect(size_t start_pos,
        in_addr_t addr,
        sp<Session> awaitingSession) {
    bool sessionAllowed = false;
    size_t j = start_pos;
    do {
        awaitingSession = mSessions.valueAt(j);
        if (awaitingSession->isRTSPServer() && !awaitingSession->isClientConnected()) {
            in_addr_t expectedAddr;
            const sp<AMessage> &srvNotifyMsg = awaitingSession->getNotificationMessage();
            if (srvNotifyMsg->findInt64("RtspClientIp", (int64_t*)&expectedAddr)) {
                ALOGI("session:%d expected IP %d.%d.%d.%d"
                    "(socket %d)",
                    awaitingSession->sessionID(),
                    (expectedAddr >> 24),
                    (expectedAddr >> 16) & 0xff,
                    (expectedAddr >> 8) & 0xff,
                    expectedAddr & 0xff,
                    awaitingSession->socket());
                ALOGI("Compare client incoming IP:%ld and expected:%ld", (long)addr, (long)expectedAddr);
                if (addr == expectedAddr || expectedAddr == INADDR_LOOPBACK) {
    ...
    for (size_t i = 0; i < mSessions.size(); ++i) {
        ...
        if (getsockname(sock, (struct sockaddr *)&sin, &len) == -1) {
            ALOGE("getsockname fail");
                ...
                (session->isRTSPServer() || session->isTCPDatagramServer() || session->isTCPServer())) {
                    ...
                    if (clientSocket >= 0) {
                        ...
                        if (err != OK) {
                            ...
                            ALOGI("incoming connection from %d.%d.%d.%d:%d "
                                  ...
                                  clientSocket);
#ifdef MTK_CROSSMOUNT_SUPPORT
                             // Find that who is witing for this incoming connection of RTSP server
                             sp<Session> awaitingSession = session;
                             if (session->isRTSPServer()) {
                                 if (!isClientAllowToConnect(i, addr, awaitingSession)) {
