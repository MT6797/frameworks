...
ASFErrorType ASFExtractor::GetNextMediaPayload(uint8_t* aBuffer,
   ...
   //for http wmv file error handling
    if(pNextPacket->payload_count > 0xff)
    {
        ALOGE("GetNextMediaPayload: file doesn't not comply to spec:payload count is too large, return ASF_ERROR_MALFORMED");
