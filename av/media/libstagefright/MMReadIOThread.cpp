...
{
    ALOGD("MMReadIOThread::startRIOThread - %p, startpos %lld, frmsize %d, cnt %d, run %d",
          this, (long long)firstFramePos,  frameSize, cacheCount, mRunning);
...
void MMReadIOThread::RioThreadEntry()
{
    ...
    while (!mStopped)
    {
        ...
        if (emptysize >= mAlignReadSize)
        {
            ...
            {
                ALOGV("threadEntry running1, Align %d, R %lld, W %lld, BSize %d, Empty %zu",
                      mAlignReadSize, (long long)mTmpReadPtr, (long long)mWritePtr, mInnerBufferSize, emptysize);
                readsize = mDataSource->readAt(mCurrentPos, (char *)mInnerBuffer->data() + mWritePtr, mAlignReadSize);
                if (((int64_t)readsize > mAlignReadSize) || (readsize < 0)) {
                    readsize = 0;
                }
        ...
        {
            ALOGV("threadEntry running2, Align %d, R %lld, W %lld, BSize %d, Empty %zu",
                  mAlignReadSize, (long long)mTmpReadPtr, (long long)mWritePtr, mInnerBufferSize, emptysize);
            continue;
        }
...
ssize_t MMReadIOThread::ReadBitsteam(void *data, ssize_t size)
{
    if ((size <= 0) || ((size_t)size > mRioFrameSize))
    {
        ALOGE("ReadBitsteam mRioFrameSize %d readsize error: %zd", mRioFrameSize, size);
        return 0;
    }

    if (data == NULL)
    {
        ALOGE("ReadBitsteam dataptr is Null %zd", size);
        return 0;
    }
    ...
    ALOGV("ReadBitsteam - size %zd, Cache %zu, mRioStat %d", size, GetReadySize(), mRioStat);

    while (GetReadySize() < (size_t)size)  // if data is not enough
    {
    ...
    ALOGV("ReadBitsteam req + %zd, R %lld, W %lld, BSize %d, Add %d, Cache %d",
          size, (long long)mReadPtr, (long long)mWritePtr, mInnerBufferSize, mAlignAddSize, mCachedSize);

    if (mReadPtr + size <= mWritePtr)
    {
        memcpy(data, (char *)mInnerBuffer->data() + mReadPtr, size);
        return size ;
    }
    else // mReadPtr +  size > mWritePtr
    {
        ...
        if (mRioStat == ERROR_END_OF_STREAM)
        {
            if (mWritePtr >= mReadPtr)
            {
                ...
                {
                    memcpy(data, (char *)mInnerBuffer->data() + mReadPtr, tempcachesize);
                }
                else
                {
                    memcpy(data, (char *)mInnerBuffer->data() + mReadPtr, size);
            ...
            else if ((mReadPtr + size) <= (off_t)(mInnerBufferSize + mAlignAddSize))  // mWritePtr < mReadPtr
            {
                memcpy(data, (char *)mInnerBuffer->data() + mReadPtr, size);
            }
            else /// mWritePtr < mReadPtr
            {
                memcpy(data, (char *)mInnerBuffer->data() + mReadPtr, mInnerBufferSize + mAlignAddSize - mReadPtr);

                if (tempcachesize <= size)
                {
                    memcpy((char *)data + mInnerBufferSize + mAlignAddSize - mReadPtr, mInnerBuffer->data(), mWritePtr);
                }
                else
                {
                    memcpy((char *)data + mInnerBufferSize + mAlignAddSize - mReadPtr, mInnerBuffer->data() ,
        ...
        if ((mReadPtr + size) <= (off_t)(mInnerBufferSize + mAlignAddSize))
        {
            memcpy(data, (char *)mInnerBuffer->data() + mReadPtr, size);
            return size;
        }
...
{
    ALOGD("ResetReadioPtr 0x%llx, r %lld, w %lld", (long long)frameStartPos, (long long)mReadPtr, (long long)mWritePtr);
    Mutex::Autolock autoLock(mLock);
...
{
    ALOGV("UpdateReadPtr- %zu, r %lld, w %lld, Cache %d, add %d",
            size, (long long)mReadPtr, (long long)mWritePtr, mCachedSize, mAlignAddSize);
    Mutex::Autolock autoLock(mLock);
    ...
    if ((GetEmptySize() >= (mInnerBufferSize / 4)) ///mInnerBufferSize/2)
            && (mRioStat != ERROR_END_OF_STREAM))
    {
        ALOGD("Signal to read %zu", GetEmptySize());
        mBufferEmptyed.signal();
    }
    ...
    {
        ALOGW("UpdateWritePtr r _error+ %zu, r %lld, w %lld, Cache %d, bsize %d, pos 0x%llx, add %d",
              size, (long long)mReadPtr, (long long)mWritePtr, mCachedSize,
              mInnerBufferSize, (long long)mCurrentPos, mAlignAddSize);
        assert(0);
    }
...
{
    ALOGV("UpdateWritePtr- %zu, r %lld, w %lld, Cache %d, pos 0x%llx, add %d",
          size, (long long)mReadPtr, (long long)mWritePtr, mCachedSize, (long long)mCurrentPos, mAlignAddSize);
    Mutex::Autolock autoLock(mLock);
    ...
    if (mCachedSize > (mInnerBufferSize + mAlignAddSize))
    {
        ALOGW("UpdateWritePtr w _error+ %zu, r %lld, w %lld, Cache %d, bsize %d, pos 0x%llx, add %d",
              size, (long long)mReadPtr, (long long)mWritePtr, mCachedSize,
              mInnerBufferSize, (long long)mCurrentPos, mAlignAddSize);
        assert(0);
    }

    ALOGV("UpdateWritePtr+ %zu, r %lld, w %lld, Cache %d, pos %llx, add %d",
          size, (long long)mReadPtr, (long long)mWritePtr, mCachedSize, (long long)mCurrentPos, mAlignAddSize);
    mBufferFilled.signal();
