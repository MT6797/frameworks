...
namespace android {
 ...
 status_t ADPCMWriter::addSource(const sp<MediaSource> &source)
 {
    ...
    ALOGD("sizeof short int = %d, int = %d, long int = %d, long long int = %d",
...
//      mResumed= ADPCM_TRUE;
 ...
 status_t ADPCMWriter::threadFunc()
 {
    ...
    while(mStoped == ADPCM_FALSE)
    {
        ...
        ALOGV("mEstimatedSizeBytes is %d", mEstimatedSizeBytes);
...
*/
    ...
    ALOGD("value1 is %d", 4 + 26 + mWaveHeader.extra_data_size + 20 + fileDataSize);
...
*/
    ...
    ALOGD("value5 is %d", fileDataSize);
 ...
 void ADPCMWriter::moveFilePointer(long offset, int fromwhere)
 {
    ...
    else
    {
        if((off = ftell(mFile)) == -1)
        {
            ...
            ALOGD("offset from file header is %d", off);
    }
 }
