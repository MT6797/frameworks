...
MPEG4FileCacheWriter::MPEG4FileCacheWriter(int fd,size_t cachesize)
{
    ...
    FCWLOGD("value_cluster=%lld", value_cluster);
    if(value_cluster > 0)
    ...
    if(value >= 0)
    {
        ...
        FCWLOGD("[@RECORD_TEST]write.delay.freq = %lld", mTestDelayFreq);
    }
    ...
    if(value >= 0)
    {
        ...
        FCWLOGD("[@RECORD_TEST]write.delay.time.us = %lld", mTestDelayTimeUs);
    }
#endif
#endif
...
MPEG4FileCacheWriter::~MPEG4FileCacheWriter()
{
    ...
    FCWLOGD("write %lld times using %lld us, average is %lld us, max is %lld us", mTimesofwrite, mTotaltime, mTotaltime/mTimesofwrite, mMaxtime);
#endif
}
...
//
        ...
        {
            FCWLOGE("file system write return error!!!Notify APP to stop record, write %d bytes, but return %d", mCacheSize-mClusterAlignLeftSize, real_write);
        ...
        if(durtime > mMaxtime)
            mMaxtime = durtime;
        mTimesofwrite++;
        FCWLOGV("[PERFORMANCE]write %dk, consume time us = %7lld, line %d", real_write/1024, durtime,__LINE__);
#endif
...
//
    ...
    //}
        ...
        while (ret >= mCacheSize)
        {
            ...
            {
                FCWLOGE("file system write return error!!!Notify APP to stop record, write %d bytes, but return %d", mCacheSize, real_write);
#ifdef USE_FILE_CACHE
                mOwner->notify(MEDIA_RECORDER_EVENT_ERROR, MEDIA_RECORDER_ERROR_UNKNOWN, 0);
#endif
...
status_t MPEG4FileCacheWriter::flush()
{
    ...
    if (mDirtySize)
    {
        if (mpCache)
        {
            ...
            if (ret != mDirtySize)
            {
                FCWLOGE("file system write return error!!!Notify APP to stop record, write %d bytes ,but return %d", mDirtySize, ret);
#ifdef USE_FILE_CACHE
                mOwner->notify(MEDIA_RECORDER_EVENT_ERROR, MEDIA_RECORDER_ERROR_UNKNOWN, 0);
#endif
...
//for test
    ...
    FCWLOGI("MPEG4FileCacheWriter::close,cache size = %dk, write speed = %lld KByte/second", mCacheSize/1024, times_of_write * mCacheSize/1024 * 1000000/total_time); //kbyte/second
#endif
    return ret;
}
