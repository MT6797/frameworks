...
#ifdef MTK_CAM_ADVANCED_EIS_SUPPORT
...
#if defined(MTK_AOSP_ENHANCEMENT) && defined(MTK_CAM_ADVANCED_EIS_SUPPORT)
...
#if defined(MTK_AOSP_ENHANCEMENT) && defined(MTK_CAM_ADVANCED_EIS_SUPPORT)
    ALOGD("EISStopVideoCount = %d", EISStopVideoCount);
#endif
...
void MPEG4Writer::Track::init() {
    ...
    if (!mIsAudio ) {
        ...
        if (mMeta->findInt32(kKeySlowMotionSpeedValue, &slowMotionSpeedValue)){
            ALOGD("Track init get kKeySlowMotionSpeedValue %d", slowMotionSpeedValue);
            mSlowMotionSpeedValue = slowMotionSpeedValue;
        }else{
            ALOGV("init not find kKeySlowMotionSpeedValue");
            mSlowMotionSpeedValue = -1;
        }
        int32_t SlowMotionTag = 0;
        if (mMeta->findInt32(kKeySlowMotionTag, &SlowMotionTag)) {
            ALOGD("Track init get kKeySlowMotionTag %d", SlowMotionTag);
            mSlowMotionSpeedTag= SlowMotionTag;
        } else {
            ALOGV("not find slowmotion tag");
...
void MPEG4Writer::Track::initStart(MetaData *params) {
    ...
    if (!mIsAudio ) {
        int32_t slowMotionSpeedValue = 0;
        if (params && params->findInt32(kKeySlowMotionSpeedValue, &slowMotionSpeedValue)){
            ALOGD("Track initStart kKeySlowMotionSpeedValue %d", slowMotionSpeedValue);
            mSlowMotionSpeedValue = slowMotionSpeedValue;
        }
        int32_t SlowMotionTag = 0;
        if (params && params->findInt32(kKeySlowMotionTag, &SlowMotionTag)) {
            ALOGD("Track initStart kKeySlowMotionTag%d", SlowMotionTag);
            mSlowMotionSpeedTag= SlowMotionTag;
        }
    }
    else{
...
#ifdef MTK_CAM_ADVANCED_EIS_SUPPORT
        int64_t EISQueueFrames = 0;
        int64_t EISDelayTimeUs = 0;
        EISQueueFrames = mOwner->getEIS();
        if (EISQueueFrames > 0 && !mIsAudio) {
            if (mVideoFrameRate > 0){
                EISDelayTimeUs = EISQueueFrames*1000000/mVideoFrameRate;
                ALOGD("find video frame rate(%d), EISDelayTimeUs = %" PRId64 "us", mVideoFrameRate, EISDelayTimeUs);
                resumeTimeStampUs = resumeTimeStampUs + EISDelayTimeUs;
                ALOGD("find %s resumeTimeStampUs = %" PRId64 "us", mIsAudio ? "A" : "V", resumeTimeStampUs);
            } else {
                ALOGW("Not find mVideoFrameRate, may cause video short than audio!");
            }
        }
#endif
