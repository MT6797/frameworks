...
status_t StagefrightRecorder::start() {
    ...
    char parameis[PROPERTY_VALUE_MAX];
    int64_t value;
    property_get("media.param.eis", parameis, "0");
    value = atol(parameis);
    ALOGD("media.param.eis = %" PRId64 "", value);
    if(value > 0) {
        if (mMP4Writer == NULL) {
            ALOGW("mWriter is NULL!");
            return UNKNOWN_ERROR;
        }
        mMP4Writer->setEIS(&value);
    }
#endif
#endif
