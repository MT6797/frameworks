...
void AudioPort::loadInChannels(char *name)
{
    ...
    audio_channel_mask_t voiceChannelMask = 0;
    for (size_t i = 0; i < mChannelMasks.size(); i ++)
    {
        switch(mChannelMasks[i])
        {
        case AUDIO_CHANNEL_IN_VOICE_UPLINK:
        case AUDIO_CHANNEL_IN_VOICE_DNLINK:
            voiceChannelMask |= mChannelMasks[i];
            break;
        }
    }

    if (voiceChannelMask == (AUDIO_CHANNEL_IN_VOICE_UPLINK | AUDIO_CHANNEL_IN_VOICE_DNLINK))
    {
        mChannelMasks.add(voiceChannelMask);
    }
#endif
}
