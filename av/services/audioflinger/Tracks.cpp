...
#ifdef MTK_AUDIO
    ALOGD("track(%p): mIsOut %d, mFrameCount %d, mSampleRate %d, mFormat %d, mChannelCount %d, mTrackCount %d, thread %p, sessionId %d",
...
bool AudioFlinger::PlaybackThread::Track::isReady() const {
    ...
    if (mFillingUpStatus != FS_FILLING || isStopped() || isPausing()) {
        ALOGD_IF( !isReady, "Track(%d) isReady = %d", mName, true );
        isReady = true;
        return true;
    }
    ...
    ALOGD_IF( isReady, "Track(%d) isReady = %d", mName, false );
    isReady = false;
    return false;
}
#endif
