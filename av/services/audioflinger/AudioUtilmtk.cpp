...
size_t fwrite_s( const void * ptr, size_t size, size_t count, FILE * stream )
{
    ...
    if ( ret != count )
    {
        ...
        // ALOGE("fwrite_s fail, error code:%d", err);
    }
    return ret;
}

int fseek_s( FILE * stream, long int offset, int origin )
{
    ...
    {
        int err = ferror ( stream );
        // ALOGE("fseek_s fail, error code:%d", err);
    }
    return ret;
}
...
size_t fread_s ( void * ptr, size_t size, size_t count, FILE * stream )
{
    ...
    {
        int err = ferror ( stream );
        // ALOGE("fread_s fail, error code:%d", err);
    }
    return ret;
}
...
void UpdateWaveHeader( FILE *fp, AudioDumpBuffer audioBuffer )
{
    ...
    else
    {
        ...
        fread_s( (char*)tmpBuffer, 46, 1, fp );
...
void WiteAudioHeader( String8 filePath, AudioDumpBuffer audioBuffer )
{
    ...
    fread_s ( &lastFileInfo, sizeof(AudioDumpFileInfo), 1, fp );
...
void PushBufferFromeQueueToVector( String8 filePath, AudioQueueBuffer *newQueueBuffer )
{
    ...
    if ( newQueueBuffer->AudioDumpBufferCount != 1 &&
         newQueueBuffer->AudioDumpBufferCount != AUDIO_DUMP_BUFFER_COUNT_MAX )
    {
        // Resize the buffer
        void *pBufTemp = newInBuffer->pBufBase;

        newInBuffer->pBufBase = (char*)malloc(newInBuffer->fileInfo.size);
        if ( newInBuffer->pBufBase != NULL )
        {
            memcpy( (char*)newInBuffer->pBufBase, pBufTemp, newInBuffer->fileInfo.size );
        }
        else
        {
            ALOGE("malloc newInBuffer->pBufBase %d fail!", newInBuffer->fileInfo.size );
        }
        free( pBufTemp );
        pBufTemp = NULL;
    }
    ...
    if ( pDumpBufferVector->size() == 0 ) // No previous buffer info
    {
        ...
        if( fp != NULL )
        {
            ...
            if ( fileLength >= sizeof(AudioDumpFileInfo) )
            {
                // Get the last info
                fseek_s (fp, fileLength - sizeof(AudioDumpFileInfo), SEEK_SET);
                fread_s ( &(lastInfoBuffer->fileInfo), sizeof(AudioDumpFileInfo), 1, fp );
...
void ProcessBufferQueue( bool bFlush )
{
    ...
    for ( int i = 0; i < keyVector.size(); i ++ )
    {
        ...
        while ( queueSize >= 2 && AUDIO_DUMP_BUFFER_COUNT_MAX > 1 )
        {
            ...
            //if ( !bFlush && ( (*newQueueBuffer).AudioDumpBufferCount + queueSize < AUDIO_DUMP_BUFFER_COUNT_MAX ) )
            //{
            //    break;
            //}
...
void *AudioDumpThread(void *arg)
{
    ...
    bool bFlush = false;
    int iNoDataCount = 0;
    ALOGV("AudioDumpThread(%p) start", hAudioDumpThread);
...
void AudioDump::dump(const char * filepath, void * buffer, int count,const char * property)
{
    ...
    { // mAudioDumpKeyMutex region
        ...
        if ( index < 0 ) // new add
        {
            property_get(af_dump_log, value, "0");
            int bflag=atoi(value);
                                 ...
                                 audio_format_t format, uint32_t sampleRate, uint32_t channelCount )
{
    ...
    {
        char value[PROPERTY_VALUE_MAX];
        property_get(AUDIO_DUMP_FILE_DELAY_TIME_KEY, value, "10");
