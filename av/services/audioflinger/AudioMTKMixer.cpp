...
{
    ALOGV("AudioFlinger::MixerThread:  constructor mSampleRate = %d",mSampleRate);
...
void AudioMTKMixer::setParameter(int name, int target, int param, void *value)
{
    ...
    ALOGV("AudioMTKMixer::setParameter name %d, target 0x%x,param 0x%x",name,target,param);
    switch (target) {
    ...
    case TRACK:
        ...
        case DO_POST_PROC:
            ...
            ALOGD("DC_REMOVE %d", track.IsMixerDoPostProc);
            break;
#ifdef TIME_STRETCH_ENABLE
        case DO_TIMESTRETCH:
...
{
    ALOGV("AudioMTKMixer::track_t::setResampler sampleRate %d,value %d,devSampleRate = %d,resampler=%p,this %p",sampleRate,value,devSampleRate,resampler,this);
...
//ALOGD("AudioMTKMixer::process__genericResampling+");
    ...
    while (e0) {
        ...
        while (e1) {
            ...
            {
                ALOGD("Do DRC and volume control");
