...
{
    SLOGD("init outsampleRate %u, intsampleRate %u",mSampleRate, SrcSampleRate);
...
size_t AudioResamplerMtk::resample(int32_t* out, size_t outFrameCount,
    ...
    if(mOutputBuf == NULL) // size of this buffer can only be determined at this time.
    {
        ...
        if(ret != OK)
        {
            SLOGE("resample fail to configBuf, There is no memory");
            return 0;
        }
    }
    ...
    while(outFrameCount){
        if(mBuffer.frameCount == 0)
        {
            ...
            if (mBuffer.raw == NULL) {
                SLOGD("AudioResamplerMtk::resample() can't get buffer");
                reset();
