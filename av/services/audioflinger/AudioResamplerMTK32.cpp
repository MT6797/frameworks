...
{
    SLOGD("init outsampleRate %u, insampleRate %u", mSampleRate, SrcSampleRate);
    ...
    SLOGD("init inChannelCount %d, OutChannelCount", mChannelCount,OutChannelCount);
    if(SrcSampleRate) {
    ...
    if(mBliSrc != NULL)
    {
        ALOG_ASSERT(!mBliSrc->initCheck(),"Fail to open Blisrc");
...
size_t AudioResamplerMtk32::resample(int32_t* out, size_t outFrameCount,
    ...
    while(outFrameCount){
        ...
        SLOGV("starting resample outFrameCount %d, outputSampleCount %d, inFrameCount %d",
    ...
    #ifdef MTK_BESSURROUND_ENABLE
        SLOGV(" mChannelCount %d, out_frame_size %d", mChannelCount, out_frame_size);
    size_t outBuffSize = outFrameCount *outChannelCount*out_frame_size;
        ...
        if(mBliSrc)
        {
            SLOGV("+resample inBufSize %d, outBufSize %d",inBufSize,outBuffSize);
#ifdef MTK_BESSURROUND_ENABLE
...
#endif
            SLOGV("-resample consume %d, inBufSize %d, outBufSize %d",consume,inBufSize,outBuffSize);
        }
        else
        {
            SLOGE("Can not Resampler, pad 0");
            consume = inBufSize;
...
//ALOGD("KH_SRC %x, In %d, Out %d", mBliSrc, in[0], out[outputIndex]);
        ...
        SLOGV("outFrameCount %d,mChannelCount * out_frame_size %d ",outFrameCount,outChannelCount * out_frame_size);
#else
